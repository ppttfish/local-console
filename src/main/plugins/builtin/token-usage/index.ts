...
export const tokenUsagePlugin: Plugin = {
  ...
  version: '2.2.0',
  description:
    '统计本机 agent (omp / zcode / opencode / codex / claude / dsh / workbuddy) 的 token 用量、成本与模型分布；监控 AI 服务商订阅配额',
  ...
  registerMcpTools(server: McpServer) {
    server.tool(
      ...
      '查询 agent token 用量与成本。agent: omp|zcode|opencode|codex|claude|dsh|workbuddy|all；model: 模型名；from/to: 毫秒时间戳；granularity: hour|day|month。返回总 token、输入/输出/缓存、缓存命中率、调用次数、USD 成本、按 agent/model/day 分桶、趋势时间序列。',
      {
        agent: z
          .enum(['omp', 'zcode', 'opencode', 'codex', 'claude', 'dsh', 'workbuddy', 'all'])
          .optional()
          .default('all'),
